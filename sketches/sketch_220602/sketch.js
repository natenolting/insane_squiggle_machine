...
function preload() {
    ...
    //https://artvee.com/dl/camellia-valtevaredo/
...
function draw() {
    ...
    for (let x = 0; x < sImg.width; x++) {
        for (let y = 0; y < sImg.height; y++) {
            ...
            let currentX  = (x * factor + sImg.width / 2) - sImg.width/2;
            let currentY = (y * factor + sImg.height) - sImg.height;
            let length =   random(helper.range(1, 6)) * noise(
                  currentX,
                  currentY
              );
            let iso = new Isometric(
            ...
            //let direct = random(helper.range(1, 7));
            let direct = 1;
            ...
            if (vectorsAreInsideBounds(iso)) {
                ...
                for (var i = 0; i < 1; i+= cbLerpDist) {
                  ...
                  stroke(hsl.h, hsl.s, hsl.l * lerp(1, lMid, i), fillOpacity);
                ...
                for (var i = 0; i < 1; i+= efLerpDist) {
                  ...
                  let gaLerp = p5.Vector.lerp(gV, aV, i);
                  stroke(hsl.h, hsl.s, hsl.l * lerp(1, lLow, i), fillOpacity);
                ...
                for (var i = 0; i < 1; i+= gcLerpDist) {
                  ...
                  stroke(hsl.h, hsl.s, hsl.l * lerp(1, lHigh, i), fillOpacity);
                ...
                for (var i = 0; i < 1; i+= cdLerpDist) {
                  ...
                  stroke(hsl.h, hsl.s, hsl.l * lerp(lHigh, lMid, i), fillOpacity);
