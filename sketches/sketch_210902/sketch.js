...
function preload() {
  ...
  for (let i = 1; i <= 53; i++) {
    squiggles.push(loadImage(`../images/jelly_fish/set-1/${i}.png`));
  }
...
function doMargin() {
  ...
  let thisCW = cW * (5/8);
  let thisCH = cH * (5/8);
  let thisImages = helpers.shuffleArray(squiggles);
  rectMode(CORNER);
  fill(0, 0, 100, 100);
  noStroke();
  beginShape();
  ...
  imageMode(CENTER);
  tint(colors[0].h, colors[0].s, colors[0].l, 100);
  image(
    thisImages[0],
    thisCW * cornerOffset,
    thisCH * cornerOffset,
    thisImages[0].width * ((thisCW / 1.75) / thisImages[0].width),
    thisImages[0].height * ((thisCW / 1.75) / thisImages[0].width)
  )
  ...
  imageMode(CENTER);
  tint(colors[1].h, colors[1].s, colors[1].l, 100);
  image(
    thisImages[1],
    width - thisCW * cornerOffset,
    thisCH * cornerOffset,
    thisImages[1].width * ((thisCW / 1.75) / thisImages[1].width),
    thisImages[1].height * ((thisCW /1.75) / thisImages[1].width)
  )
  ...
  imageMode(CENTER);
  tint(colors[2].h, colors[2].s, colors[2].l, 100);
  image(
    thisImages[2],
    width - thisCW * cornerOffset,
    height - thisCH * cornerOffset,
    thisImages[2].width * ((thisCW / 1.75) / thisImages[2].width),
    thisImages[2].height * ((thisCW / 1.75) / thisImages[2].width)
  )
  ...
  imageMode(CENTER);
  tint(colors[3].h, colors[3].s, colors[3].l, 100);
  image(
    thisImages[3],
    thisCW * cornerOffset,
    height - thisCH * cornerOffset,
    thisImages[3].width * ((thisCW / 1.75) / thisImages[3].width),
    thisImages[3].height * ((thisCW / 1.75) / thisImages[3].width)
  )
}
...
function doSquiggle(x, y) {
  ...
  let thisRotation = radians(random(-10, 10));
  rotate(thisRotation);
