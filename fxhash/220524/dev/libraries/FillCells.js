class FillCells {
    ...
    range = function(low = 1, high = 10) {
      return this.helper.range(low, high);
    }
    ...
    gradient = function() {
        ...
        color1 = this.cG.color(this.colors[this.colorIndex].h, this.colors[this.colorIndex].s, this.colors[this.colorIndex].l, 100);
        this.colorIndex++;

        if (this.colorIndex > this.colors.length - 1) {
            this.colorIndex = 0;
        }
        color2 = this.cG.color(this.colors[this.colorIndex].h, this.colors[this.colorIndex].s, this.colors[this.colorIndex].l, 100);
        this.colorIndex++;

        // this.cG.fill(color1);
        // this.cG.rect(0,0, this.cell.w, this.cell.h / 2);
        // this.cG.fill(color2);
        // this.cG.rect(0, this.cell.h / 2, this.cell.w, this.cell.h / 2);

        for (let i = 0; i <= this.cell.h; i++) {
            let y = map(i, 0, this.cell.h, 0, 1);
            let lerp = lerpColor(color1, color2, y);
            this.cG.fill(lerp);
            this.cG.rect(0, i, this.cell.w, 2);
        }
    ...
    pixelated = function (splitW = 10, splitH = 10, shape = "square") {
        ...
        for (let r = 0; r < splitW; r++) {
            for (let c = 0; c < splitH; c++) {
                let thisColor = this.colors[this.colorIndex];
                this.cG.noStroke();
    ...
    pointField = function (dotCount = 1000) {
        ...
        for (let i = 0; i < dotCount; i++) {
            ...
            let color1 = this.colors[this.colorIndex];
            let x = round(map(fxrand(), 0, 1, 0, this.cell.w - 1));
            let y = round(map(fxrand(), 0, 1, 0, this.cell.h - 1));
            this.cG.fill(color1.h, color1.s, color1.l, 90);
            let range = this.range(1, 3);
            this.cG.ellipse(x, y, range[round(map(fxrand(), 0, 1, 0, range.length - 1))]);
            if (this.colorIndex >= this.colors.length - 1) {
                this.colorIndex = 0;
            } else {
                this.colorIndex++;
    ...
    cheerio = function () {
        ...
        this.cG.fill(
            this.colors[this.colorIndex].h,
            this.colors[this.colorIndex].s,
            this.colors[this.colorIndex].l,
            this.opacity
        );
        if (this.colorIndex >= this.colors.length - 1) {
            this.colorIndex = 0;
