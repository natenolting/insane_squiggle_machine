...
const CONFIG = {
    ...
    wormSize: getRandomizer()
};

const IMAGE_LIST = [
  './images/Adolphe_Millot_fleurs_D.jpg',
  './images/Adolphe_Millot_fleurs_D_cmyk.jpg',
  './images/Adolphe_Millot_fleurs_D_cmyk_invert.jpg',
];
window.preload = () => {

    img = loadImage(IMAGE_LIST[floor(map(getRandomizer(), 0, 1, 0, IMAGE_LIST.length))]);
...
function setupFactor() {
  ...
  SETTING.factor = map(getRandomizer(), 0, 1, 8, 16);
}
...
for (let c of cells) {
        ...
        let iso = new Isometric(
            ...
            map(CONFIG.wormSize, 0, 1, 3, 6) * noise(x1, y1)
        );
        ...
        iso.pickDirection(direction, map(colorAverage, 0, 255, 2, 5));
        if (isoIsInsideBounds(iso, SETTING.margin * .75)) {
          used++;

            let xDiviation = (iso.a.x - iso.b.x) * noise(c.index) * map(getRandomizer(), 0, 1, 1, 4);
