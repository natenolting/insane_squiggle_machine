...
function setupSettings() {
    ...
    settings.pixelDensityLow = 7
    settings.pixelDensityHigh = settings.pixelDensityLow * 3
...
window.preload = () => {
    ...
    for (let i = 0; i < imageIndexes.length; i++) {
      squiggles.push(loadImage(`./images/squiggles/set-21/${imageIndexes[i]}.png`));
    }
}
...
function drawCells(allCells, colorSet) {
    ...
    for (const element of allCells) {
        ...
        let colorSet = HELPER.shuffleArray(pallet['rgb']);
        let thisImage = HELPER.shuffleArray(squiggles)[0];
        let newCg = createGraphics(c.w, c.h);
        let tf = new Transform();
        let roll = randomizerRollADie(6);
        tf.cg = newCg;
        newCg.scale(c.w / thisImage.width);
        switch(roll) {
          case 1:
            tf.imgRotatePi(thisImage, -(thisImage.width/2), -(thisImage.height/2), CENTER)
            break;
          case 2:
            tf.imgRotateHalfPi(thisImage, 0, 0)
            break;
          case 3:
            tf.imgRotateHalfPiCc(thisImage, 0, 0)
            break;
          case 4:
            tf.imageFlipV(thisImage,0,0)
            break;
          case 5:
              tf.imageFlipH(thisImage,0,0)
            break;
          defaut:
        ...
        cG.image(newCg, c.x, c.y);
        cG.tint(colorSet[0][0], colorSet[0][1], colorSet[0][2])
...
window.draw = () => {
    ...
    if (USE_ISOMETRIC) {
        ...
        for (var r = 0; r <= settings.isoMaxDirections; r++) {
            if (randomizerRollADie(6) >= 4) {
                allowedDirections.push(round(map(getRandomizer(), 0, 1, 1, 7)));
            }
        }
        let randPixelPositions = HELPER.shuffleArray(pixelPositions);
        for (const element of randPixelPositions) {
            ...
            let isosSideWidth = map(p.red, 0, 255, 1, settings.isoMaxMultiplier);
            let isosSizeMultipler = map(p.green, 0, 255, 1, settings.isoMaxMultiplier);
            let isoLengthMultipler = map(p.blue, 0, 255, 1, settings.isoMaxMultiplier);

            if (randomizerRollADie(20) === 20) {
              let oa = fxrand() * 5;
              isosSideWidth = map(p.red, 0, 255, 1, settings.isoMaxMultiplier + (settings.isoMaxMultiplier * map(fxrand(), 0, 1, -oa, oa)));
              isosSizeMultipler = map(p.green, 0, 255, 1, settings.isoMaxMultiplier + (settings.isoMaxMultiplier * map(fxrand(), 0, 1, -oa, oa)));
              isoLengthMultipler = map(p.blue, 0, 255, 1, settings.isoMaxMultiplier + (settings.isoMaxMultiplier * map(fxrand(), 0, 1, -oa, oa)));
            }
