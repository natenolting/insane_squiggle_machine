...
window.draw = () => {
  ...
  let moundRowsDensity = floor(map(CONFIG.density, 0, 1, 200, 350));
  // if (!CONFIG.coloring_book) {
  // let lineColor = HELPER.shuffleArray(pallet.hsl)[0];
  // for (var i = 0; i < moundRowsDensity; i++) {
  //   let y = map(i, 0, moundRowsDensity, 0, cg.height);
  //   cg.stroke(lineColor.h, lineColor.s, lineColor.l, 25);
  //   cg.strokeWeight(map(i, 0, moundRowsDensity, 1, 35));
  ...
  for (var i = 0; i < moundRowsDensity; i++) {
    ...
    doMound(
      ...
      map(i, 0, moundRowsDensity, .125, map(noise(y), 0, 1, .375, .95)),
      radians(map(noise(y), 0, 1, -4, 4))
    );
  }
...
function doMound(x, y, s = 1, r=0) {
  ...
  let newCg = createGraphics(thisImage.width * s * 1.25, thisImage.height * s * 1.25);
  let newTransform = new Transform();
  let flip = HELPER.flipACoin();
  newTransform.cg = newCg;
  newCg.translate(newCg.width * .125, newCg.height * .125)
  newCg.scale(s);
  newCg.rotate(r);
  if (flip) {
  ...
  if (!CONFIG.coloring_book) {
    ...
    shadowCg.translate(newCg.width * .125, newCg.height * .125)
