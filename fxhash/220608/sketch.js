...
window.preload = () => {
  ...
  for (let i = 1; i <= 132; i++) {
    squiggles.push(loadImage(`./images/squiggles/set-15/${i}-fs8.png`));
  }
...
window.draw = () => {
  ...
  for (var i = 0; i < randCells.length * map(CONFIG.density, 0, 1, 0.4, 0.8); i++) {
    let c = randCells[i];
    // stroke(0,0,0,100);
    // strokeWeight(2);
...
function doSquiggle(x, y) {
  ...
  let thisImage = squiggles[floor(map(noise(x, y), 0, 1, 0, squiggles.length))];
  cg.push();
